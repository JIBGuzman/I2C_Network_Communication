...
int main(void){
	...
	UART0_Init();
	UART0_OutString("UART Initialized\r\n");
	LED_Init();
	UART0_OutString("LED Initialized\r\n");
	BTN_Init();
	UART0_OutString("BTN Initialized\r\n");
	
	#if defined(DELAY) || defined(TCS34727) || defined(MPU6050) || defined(LCD) || defined(FULL_SYSTEM)	
	WTIMER0_Init();
	UART0_OutString("Wide Timer Initialized\r\n");
	#endif
	
	#if defined (I2C) || defined(TCS34727) || defined(MPU6050) || defined(LCD) || defined(FULL_SYSTEM)
	I2C0_Init();
	UART0_OutString("I2C Initialized\r\n");
	#endif
	...
	UART0_OutCRLF();
	UART0_OutString("Entering Mode 1: DELAY TEST\r\n");
	while(1){		
