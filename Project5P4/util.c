...
uint32_t WTIMER0_GetTime(void) {
    return WTIMER0_TAR_R; // Return the current timer value
}

void DELAY_1MS(uint32_t delay){
	WTIMER0_TAILR_R = delay - 1; //Set the load value of the timer
	WTIMER0_CTL_R |= WTIMER0_TAEN_BIT; //Enable WTIMER0 Timer A
	while(WTIMER0_TAR_R != 0); //Wait until the timer has counted down
	WTIMER0_CTL_R &= ~(WTIMER0_TAEN_BIT); //Disable WTIMER0 Timer A
}
